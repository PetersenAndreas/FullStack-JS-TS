...
ServerFacade = () => {
  ...
  */
  ...
  async function nearByPlayers(userName, password, lat, lon, distance) {
    const newPosition = { "userName": userName, "password": password, "lat": lat, "lon": lon, "distance": Number(distance) }
    const config = {
      method: 'POST',
      headers: {
