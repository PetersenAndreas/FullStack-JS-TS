...
describe("Verify /gameapi/getPostIfReached", () => {
  ...
  it("Should find team2 +team3, since both are inside range", async function () {
    const newPosition = { "userName": "t1", "password": "secret", "lat": 55.77, "lon": 12.48, "distance": 10000}
    const config = {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
  ...
  it("Should NOT find team2, since not in range", async function () {
    const newPosition = { "userName": "t1", "password": "secret", "lat": 55.77, "lon": 12.48, "distance": 1}
    const config = {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
  ...
  it("Should NOT find team2, since since credentials are wrong", async function () {
    const newPosition = { "userName": "t1", "password": "wrongPassword", "lat": 55.77, "lon": 12.48, "distance": DISTANCE_TO_SEARCH}
    const config = {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
